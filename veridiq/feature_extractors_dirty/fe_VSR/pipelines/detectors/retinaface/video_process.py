...
class VideoProcess:
    ...
    def __call__(self, video, landmarks):
        if landmarks is None or not landmarks or all(lm is None for lm in landmarks):
            print("No landmarks — passing the video unaltered.")
            gray_video = [cv2.cvtColor(frame, cv2.COLOR_RGB2GRAY) for frame in video]
