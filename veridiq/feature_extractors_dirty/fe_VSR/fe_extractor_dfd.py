...
def main(split):
    ...
    path_to_features_root = "/data/av-extracted-features/avlips_auto_avsr/audiovisual"
    # path_to_features_root = f"/data/av-extracted-features/favc_auto_avsr/{modality}"
    ...
    if modality == "audio":
        ...
        path_to_crops = ("/data/avlips/AVLips", "/data/avlips/AVLips")
    ...
    # csv1_paths = load_csv_paths(f"/data/av-datasets/datasets/FakeAVCeleb_preprocessed/all_splits/{SPLIT}_split.csv")
    csv1_paths = load_csv_paths(f"/data/avlips/AVLips/test_labels.csv")
    ...
    for i, file_path in enumerate(tqdm(files)): #[start_index:end_index])):
        ...
        if modality == "audio":
            ...
            # file_path = file_path.replace(".mp4", ".wav").replace("/videos/", "/processed/")
            file_path = file_path.replace("_roi.mp4", ".wav")
