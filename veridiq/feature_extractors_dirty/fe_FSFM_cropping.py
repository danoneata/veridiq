...
def extract_align_and_save_face(frame, dst_path, save_img_name, face_predictor):
...
def extract_and_save_face(frame, dst_path, save_img_name):
    ...
    if len(faces) > 0:
        ...
        cv2.imwrite(os.path.join(dst_path, save_img_name), frame)
...
def extract_face_from_fixed_num_frames(src_video, dst_path, video_name, num_frames=None, align=True):
    ...
    for frame_index in frame_indices:
        ...
        if not ret:
            print("skip frame - read")
            continue
        save_img_name = f"{video_name}_frame_{frame_index}.png"
